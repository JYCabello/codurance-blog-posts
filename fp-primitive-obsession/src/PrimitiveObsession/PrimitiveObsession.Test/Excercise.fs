...
let ``Calculates the tax of multiple amounts`` () =
  ...
  calculator.add
  <| Incoming { Amount = 120; Currency = GBP }

  calculator.add
  <| Incoming { Amount = 200; Currency = GBP }
...
let ``Different currencies are not taxed`` () =
  ...
  calculator.add
  <| Incoming { Amount = 200; Currency = USD }
...
let ``A negative balance results in no tax`` () =
  ...
  calculator.add
  <| Outgoing { Amount = 200; Currency = GBP }

  calculator.add
  <| Outgoing { Amount = 400; Currency = GBP }

  calculator.add
  <| Outgoing { Amount = 20; Currency = GBP }
...
let ``Everything is reported in the local currency`` () =
  ...
  calculator.add
  <| Incoming { Amount = 400; Currency = GBP }

  calculator.add
  <| Outgoing { Amount = 200; Currency = USD }

  calculator.add
  <| Incoming { Amount = 200; Currency = EUR }
